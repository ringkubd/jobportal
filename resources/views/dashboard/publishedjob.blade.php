...
@include('dashboard.modal.jobdetails_modal')
    <div class="col-md-8">	  
          <h1 class="text-center"> published Job Lists</h1>
              ...
              <div class="form-group search_div">
                ...
                <div class="panel panel-default" id="listbody">
                  <div class="panel-heading">
                    <h3 class="panel-title">Published Job lists:({{$show_published_job != false?count($show_published_job):'0'}})</h3>           
                  </div>
                  <div class="panel-body" id="items">	

            			<table class="table table-responsive table-striped" id="example">
                        @if($show_published_job == false)
                        <h1 class="text-center alert-danger">NO PUBLISHED JOB YET</h1>
                        @else
                        <thead>
                        ...
                        <tbody id="searcheble">
                            @foreach($show_published_job as $show_jobdetail)
                                <tr class="odd gradeX all_data">
                                    ...
                                    <td><button class="btn btn-warning btn-sm unpublished_job" data-unpublished="{{$show_jobdetail->id}}">Unpublished</button><button class="btn btn-default btn-sm view_jobjetails"  data-toggle="modal" data-target="#jobprofile" data-view_job_details="{{$show_jobdetail->id}}">View</button></td> 
                                </tr>
...
$(document).on('click','.unpublished_job',function(){
 var job_unpub_id=$(this).attr("data-unpublished");
...
$.ajax({
        ...
        data:{'job_unpub_id':job_unpub_id,'companyemail':companyemail,'jobtitle':jobtitle,'contactperson':contactperson,'action':'unpublishedjobs'},
