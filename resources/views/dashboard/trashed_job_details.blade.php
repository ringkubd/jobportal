...
<div class="col-md-8" id="Posted_job_details">
          <h1 class="text-center">Trashed Job: {{$jobprofile_details !=false?$jobprofile_details->total($jobprofile_details):'0'}}</h1>
           ...
           <input type="hidden" name="" class="currentpage" value="{{$jobprofile_details->currentPage()}}">
            ...
            <div class="well">
                <div class="panel panel-primary">
                  <div class="panel-heading">
                   <h2 class="panel-title">Trashed Job title:<span class="jobtitle">{{$jobprofile_posted->jobtitle}}</span><a href="#" id="edit_jobs" class="pull-right" data-toggle="modal" data-target="#myModal"><i class="fa fa-edit" aria-hidden="true"></i>
</a></h2>           
                  </div>
            ...
            <div class="panel panel-primary">
                ...
                <table class="table table-responsive table-striped well">
                           <tr>
                            ...
                            <tr>
                                <th><button class="btn btn-success pull-right job_restore" value="{{$jobprofile_posted->id}}">Restore</button></th>
                                <td><button class="btn btn-danger pull-right deletejob" value="{{$jobprofile_posted->id}}">delete</button></td>
                            </tr> 
            ...
            @else
            <h1 class="text-center alert-danger">NO JOB TRASHED  YET</h1>
...
$(document).on('click','.job_restore', function() {
...
$(document).on('click','.deletejob', function() {
        ...
        var prepageurl=$('.prepageurl').val();
        var nextpageurl=$('.nextpageurl').val();
        var jobdelete_id=$(this).val();
        $.ajax({
              type:'get',
              url:'{!!URL::to('inspector/job_published')!!}',
              data:{'jobdelete_id':jobdelete_id,'action':'jobdelete'},
              success:function(data){                    
              if(currentpage==1 && currentpage==lastpage) {
              window.location.href=pathurl;
              }else if(currentpage==1 && currentpage !=lastpage){
